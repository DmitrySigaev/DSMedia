...
void xml_check_line(XMLParser *parser)
{
	...
	if (parser->line_buffer[parser->current_pos]=='#') parser->line_size = parser->current_pos;
	else if ((parser->line_buffer[parser->current_pos]=='/') && (parser->line_buffer[parser->current_pos+1]=='/') ) parser->line_size = parser->current_pos;

	/*skip comment*/
	else if ((parser->line_buffer[parser->current_pos] =='<')
...
next_line:
		...
		if (parser->unicode_type) {
			...
			*dst = 0;
			...
			dst = l;
			utf8_wcstombs(parser->line_buffer, XML_LINE_SIZE, (const unsigned short **) &dst);
		...
		if ( (parser->line_buffer[parser->current_pos]=='#') 
			|| ( (parser->line_buffer[parser->current_pos]=='/') && (parser->line_buffer[parser->current_pos+1]=='/')) )
			goto next_line;
