...
static int find_frame_rate_index(MpegEncContext *s){
    ...
    for(i=1;i<14;i++) {
        int64_t n0= 1001L/frame_rate_tab[i].den*frame_rate_tab[i].num*s->avctx->frame_rate_base;
        int64_t n1= 1001L*s->avctx->frame_rate;
        ...
        //mpeg2 aspect
            if(s->aspect_ratio_info > 1){
                if( (s1->pan_scan.width == 0 )||(s1->pan_scan.height == 0) ){
					AVRational avr = {s->width, s->height};
					...
					AVRational avr = {s1->pan_scan.width, s1->pan_scan.height};
                    s->avctx->sample_aspect_ratio= 
                        av_div_q(
                         mpeg2_aspect[s->aspect_ratio_info], 
                         avr
