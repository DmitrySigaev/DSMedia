    ...
    if(!(s->flags&CODEC_FLAG_PASS2)){
        ...
        if(s->avctx->rc_initial_cplx){
            for(i=0; i<60*30; i++){
                ...
                rcc->i_cplx_sum [rce.pict_type] += (int64_t)(rce.i_tex_bits*rce.qscale);
                rcc->p_cplx_sum [rce.pict_type] += (int64_t)(rce.p_tex_bits*rce.qscale);
                rcc->mv_bits_sum[rce.pict_type] += (int64_t)(rce.mv_bits);
...
static double get_qscale(MpegEncContext *s, RateControlEntry *rce, double rate_factor, int frame_num){
    ...
    double const_values[]={
        ...
        (int64_t)rcc->qscale_sum[pict_type] / (double)rcc->frame_count[pict_type],
        a->qcompress,
/*        rcc->last_qscale_for[I_TYPE],
        rcc->last_qscale_for[P_TYPE],
        rcc->last_qscale_for[B_TYPE],
        rcc->next_non_b_qscale,*/
        (int64_t)rcc->i_cplx_sum[I_TYPE] / (double)rcc->frame_count[I_TYPE],
        (int64_t)rcc->i_cplx_sum[P_TYPE] / (double)rcc->frame_count[P_TYPE],
        (int64_t)rcc->p_cplx_sum[P_TYPE] / (double)rcc->frame_count[P_TYPE],
        (int64_t)rcc->p_cplx_sum[B_TYPE] / (double)rcc->frame_count[B_TYPE],
        (int64_t)(rcc->i_cplx_sum[pict_type] + rcc->p_cplx_sum[pict_type]) / (double)rcc->frame_count[pict_type],
        0
    };
...
static void adaptive_quantization(MpegEncContext *s, double q){
    ...
    float bits_sum= 0.0;
    float cplx_sum= 0.0;
    float *cplx_tab = (float *)av_malloc(sizeof(float)*s->mb_num);
    float *bits_tab = (float *)av_malloc(sizeof(float)*s->mb_num);
...
//printf("%f %f %f last:%d var:%d type:%d//\n", q, rce->new_qscale, br_compensation, s->frame_bits, var, pict_type);
        ...
        rcc->i_cplx_sum [pict_type] += (int64_t)(rce->i_tex_bits*rce->qscale);
        rcc->p_cplx_sum [pict_type] += (int64_t)(rce->p_tex_bits*rce->qscale);
        rcc->mv_bits_sum[pict_type] += (int64_t)(rce->mv_bits);
...
//        printf("%f %d %f\n", expected_bits, (int)all_available_bits, rate_factor);
        if(expected_bits > (int64_t)all_available_bits) rate_factor-= step;
    }
    av_free(qscale);
    av_free(blured_qscale);

    if(abs(expected_bits/(int64_t)all_available_bits - 1.0) > 0.01 ){
