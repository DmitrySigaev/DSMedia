...
void h263_encode_picture_header(MpegEncContext * s, int picture_number)
{
    ...
    if(s->h263_plus){
        for(i=0; i<2; i++){
            int div, error;
            div= (s->avctx->frame_rate_base*1800000L + 500L*s->avctx->frame_rate) / ((1000L+i)*s->avctx->frame_rate);
            div= clip(1, div, 127);
            error= ABS(s->avctx->frame_rate_base*1800000L - (1000L+i)*s->avctx->frame_rate*div);
            if(error < best_error){
                best_error= error;
                best_divisor= div;
                best_clock_code= i;
