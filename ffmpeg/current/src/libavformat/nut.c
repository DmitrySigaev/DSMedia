...
#define     MAIN_STARTCODE (0x7A561F5F04ADUL + (((uint64_t)('N'<<8) + 'M')<<48)) 
#define   STREAM_STARTCODE (0x11405BF2F9DBUL + (((uint64_t)('N'<<8) + 'S')<<48)) 
#define KEYFRAME_STARTCODE (0xE4ADEECA4569UL + (((uint64_t)('N'<<8) + 'K')<<48)) 
#define    INDEX_STARTCODE (0xDD672F23E64EUL + (((uint64_t)('N'<<8) + 'X')<<48)) 
#define     INFO_STARTCODE (0xAB68B596BA78UL + (((uint64_t)('N'<<8) + 'I')<<48)) 
...
//av_log(NULL, AV_LOG_DEBUG, "put_v()= %lld\n", val);
    val &= 0x7FFFFFFFFFFFFFFFUL; // FIXME can only encode upto 63 bits currently
    i= get_length(val);
