...
static M4INLINE u16 convert16 (u8 *p)
{
#ifdef M4_BIG_ENDIAN
  return *(u16 *)p;
#else
...
static M4INLINE u32 convert32 (u8 *p)
{
#ifdef M4_BIG_ENDIAN
  return *(u32 *)p;
#else
...
static Bool mpeg2ps_stream_find_ac3_frame (mpeg2ps_stream_t *sptr)
{
  ...
  while (AC3_ParseHeader(sptr->pes_buffer + sptr->pes_buffer_on,
			      sptr->pes_buffer_size - sptr->pes_buffer_on,
			       &diff, 
			      &hdr) <= 0) {
    // don't have frame
...
static Bool mpeg2ps_stream_find_mp3_frame (mpeg2ps_stream_t *sptr)
{
  ...
  while ((hdr=MP3_GetNextHeaderMem(sptr->pes_buffer + sptr->pes_buffer_on,
			sptr->pes_buffer_size - sptr->pes_buffer_on,
			&diff) ) == 0) {
    // don't have frame
    if (sptr->pes_buffer_size > 3) {
  ...
  // have frame.
  sptr->frame_len = MP3_GetFrameSize(hdr);
  sptr->pes_buffer_on += diff;
  if (diff == 0 && started_new_pes) {
				 ...
				 u32 buflen)
{
  ...
  if (sptr->m_stream_id >= 0xc0) {
    ...
    sptr->channels = MP3_GetNumChannels(hdr);
    sptr->freq = MP3_GetSamplingRate(hdr);
    sptr->samples_per_frame = MP3_GetSamplesPerFrame(hdr);
    sptr->bitrate = MP3_GetBitRate(hdr) * 1000; // give bps, not kbps
    sptr->layer = MP3_GetLayer(hdr);
  } else if (sptr->m_stream_id == 0xbd) {
    if (sptr->m_substream_id >= 0xa0) {
      // PCM - ???
		...
		u32 pos;
		AC3Header hdr;
      AC3_ParseHeader(buffer, buflen, &pos, &hdr);
