...
void CB_UnlockInput(LPCOMPOBUF cb, u32 TS, u32 NbBytes)
{
	...
	if (cu) {
		...
		if ((cb->odm->codec->type==M4ST_VISUAL) && cb->odm->mo && cb->odm->mo->num_open) {
			Term_InvalidateScene(cb->odm->term);
		}
	}
	CB_Lock(cb, 0);
}
...
LPCUBUFFER CB_GetOutput(LPCOMPOBUF cb)
{
	...
	case CB_PAUSE:
		...
		if (cb->odm->codec->type != M4ST_VISUAL) goto exit;
		break;
	}
...
void CB_DropOutput(LPCOMPOBUF cb)
{
	...
	if (cb->output->dataLength && (cb->odm->codec->type == M4ST_VISUAL) ) {
...
Bool CB_IsRunning(LPCOMPOBUF cb)
{
	...
	if ((cb->Status == CB_BUFFER_DONE) && CK_IsStarted(cb->odm->codec->ck)) {
		cb->Status = CB_PLAY;
		return 1;
	}

	if ((cb->odm->codec->type == M4ST_VISUAL)
		&& (cb->Status == CB_STOP)
